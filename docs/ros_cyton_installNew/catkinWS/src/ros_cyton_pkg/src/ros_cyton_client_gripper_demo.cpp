...
//
/// @file ros_cyton_client_gripper_demo.cpp
//
//------------------------------------------------------------------------------
...
// %EndTag(LOOP_RATE)%
  ...
  while (ros::ok())
  {
    ...
    double val_array1[6]={-0.0139591,0.0235977,0.712172,0,0,0};
    
    ///Sending to endeffector to a random position
    Send_EE_Pose("point_end_effector",val_array1);
    ///Open Gripper(Value from GUI)
    Send_Gripper_Value(0.015);	

    ros::spinOnce();
    loop_rate.sleep();
    ///Sending to endeffector to the previous pose
    Send_EE_Pose("point_end_effector",val_array1);
    ///Close Gripper
    Send_Gripper_Value(0);	
    ros::spinOnce();
    loop_rate.sleep();
